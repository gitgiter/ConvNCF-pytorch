...
class BPRLoss(nn.Module):
    ...
    def forward(self, pos_preds, neg_preds):
        ...
        regular = 0
        # loss = torch.sum(torch.log(self.sigmoid(-distance)), dim=1) + regular
#         loss = torch.sum(torch.log((1 + torch.exp(-distance))), dim=1) + regular
        loss = torch.sum(torch.log((1 + torch.exp(-distance)))) + regular
...
def train():
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.2, last_epoch=-1)
